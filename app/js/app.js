...
module.exports = function (worksheetParser, segment) {
  ...
  var setBlankWorksheet = function () {
    ...
    ws.segments().push(
      ...
      ws.segment.freeSegment("# Gorilla REPL\n\nWelcome to gorilla :-)\n\nShift + enter evaluates code. " +
  ...
  self.saveDialog = saveDialog((function (f) {
    self.handleSaveDialogSuccess(f)
  }));
    ...
    $.get(cljserver + "/load", {"worksheet-filename": filename})
      .done(function (data) {
        if (data['worksheet-data']) {
          ...
          var codeSegments = _.filter(self.worksheet().segments(), function (s) {
            return s.type === 'code'
          });
  ...
  eventBus.on("app:commands", function () {
    var visibleCommands = commandList.filter(function (x) {
      return x.showInMenu
    });
